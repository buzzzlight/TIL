...
정해진 기준이 없음 (나누는 기준이 회사마다 사람마다 다름)
...
- 인터페이스를 구성하는 기본 단위를 명확하게 구분할 수 있기 떄문에 디자인 시스템을 일관성 있게 유지하며 개발을 더욱 효율적으로 할 수 있음
- 컴포넌트를 재사용할 수 있으며 수정이 용이하여 유지보수에 용이함
...
- 내가 제어하고 내가 push 하고 진실의 원천으로 계속 상태가 유지된다
- 렌더링될 때 고유의 값이 유지되고 내가 입력하는 상태값이 같고 리액트 상태가 같고 사용자의 입력값이 같다
...
- 렌더링되는 값과 사용자의 눈에 보이는 값과 리액트 useRef 가변 컨테이너의 값이 다르다
...
- 값 유효성 체크를 해야하는 경우(비밀번호)에 제어 컴포넌트를 사용하고 값 유효성 체크를 해야할 필요가 없이 단순히 값을 전달하는 경우엔 비제어 컴포넌트를 사용한다
- 폼 작성 시, 타이핑마다 유효성 체크를 해야한다면 제어 컴포넌트 / form 제출 시에 한번만 유효성 체크를 한다면 비제어 컴포넌트
...
- 테스트 도구 - jest
  - create react app 시 jest와 rtl(react testing library) 자동으로 설치됨
