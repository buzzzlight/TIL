...
for movies in movie:
    print(movies.director, movies.genre, movies.title, movies.opening_date, movies.running_time, movies.screening)
```
...
movie = Movie.objects.all()
for movies in movie:
    director_ = Director.objects.get(id = movies.director_id)
    print(director_.name, movies.genre, movies.title, movies.opening_date, movies.running_time, movies.screening)
```

```python
movie = Movie.objects.all()
for movies in movie:
    print(movies.director.name, movies.genre, movies.title, movies.opening_date, movies.running_time, movies.screening)
```
...
for movies in movie:
    director_ = Director.objects.get(id = movies.director_id)
    genre_ = Genre.objects.get(id = movies.genre_id)
    print(director_.name, genre_.title, movies.title, movies.opening_date, movies.running_time, movies.screening)
```
...
```python
bjh = Director.objects.get(name = '봉준호').id
movie = Movie.objects.filter(director=bjh).order_by('opening_date')
...
```python
bjh = Director.objects.get(name = '봉준호').id
movies = Movie.objects.filter(director=bjh).order_by('opening_date')[1]
...
movie = Movie.objects.filter(director = bjh, genre = drama).order_by('opening_date')
...
```python
bjh = Director.objects.get(name = '봉준호').id
movie = Movie.objects.exclude(director = bjh).order_by('opening_date')
